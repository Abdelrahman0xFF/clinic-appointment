        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 lg:p-8">
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-lg p-5 mb-8">
                <div class="flex gap-3">
                    ...
                    <div class="space-y-3">
                        <p class="font-semibold text-slate-900">Please transfer the consultation fee:</p>
                        <div>
                            <p class="text-sm font-medium text-slate-700 mb-1">InstaPay:</p>
                            <div class="flex items-center gap-2 bg-white px-3 py-2 rounded border border-amber-200">
                                <code class="text-sm font-mono text-slate-900 flex-1 truncate">{{ instapayLink }}</code>
                                <button type="button" (click)="copyText(instapayLink, 'instapay')"
                                        class="text-amber-600 hover:text-amber-700 transition cursor-pointer shrink-0">
                                    <ng-icon [name]="copiedKey === 'instapay' ? 'fluentCheckmark' : 'fluentCopy'" size="16" />
                                </button>
                            </div>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-700 mb-1">Vodafone Cash:</p>
                            <div class="flex items-center gap-2 bg-white px-3 py-2 rounded border border-amber-200">
                                <code class="text-sm font-mono text-slate-900 flex-1 truncate">{{ walletNumber }}</code>
                                <button type="button" (click)="copyText(walletNumber, 'wallet')"
                                        class="text-amber-600 hover:text-amber-700 transition cursor-pointer shrink-0">
                                    <ng-icon [name]="copiedKey === 'wallet' ? 'fluentCheckmark' : 'fluentCopy'" size="16" />
                                </button>
                            </div>
                        </div>
            ...
            <div>
                <span class="block text-sm font-semibold text-slate-900 mb-3">Upload Payment Screenshot *</span>
                @if (!receiptFileName) {
                    <div tabindex="0" role="button"
                         [class]="'border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors outline-none focus:ring-2 ' + (showErrors && !receiptFileName ? 'border-red-300 focus:border-red-400 focus:ring-red-100' : 'border-slate-300 hover:border-blue-500 focus:border-blue-400 focus:ring-blue-100')"
                    ...
                    <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-center justify-between">
                        <div class="flex items-center gap-3 min-w-0">
                            <ng-icon name="fluentCheckmarkCircle" size="20" class="text-emerald-600 shrink-0" />
                            <div class="min-w-0">
                                <p class="font-medium text-slate-900 text-sm truncate">{{ receiptFileName }}</p>
                                <p class="text-xs text-slate-500">{{ receiptFileSizeKB }} KB</p>
                            </div>
                        </div>
                        <button type="button" (click)="removeReceipt.emit(); showErrors = false"
