    ...
    template: `
        <div class="rounded-xl border border-slate-200 bg-white p-6 h-full flex flex-col">
            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                <span
                    class="flex items-center justify-center size-10 rounded-xl bg-amber-100 text-amber-600"
                >
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                (input)="updateLink(item.key, $any($event.target).value)"
                                placeholder="https://..."
                            ...
                            @if (item.value?.link) {
                                <span
                                    class="absolute right-3 top-1/2 -translate-y-1/2 size-5 rounded-full bg-blue-100 flex items-center justify-center cursor-pointer"
                                    (click)="updateLink(item.key, '')"
                                    (keydown.enter)="updateLink(item.key, '')"
                                    tabindex="0"
                                >
...
export class SettingsSocial {
    @Input() socialMedia: Record<string, { link: string } | null> = {};
    @Output() socialMediaChange = new EventEmitter<Record<string, { link: string } | null>>();

    updateLink(key: string, link: string) {
        const updated = { ...this.socialMedia };
        if (link) {
            updated[key] = { link };
        } else {
            updated[key] = null;
        }
        this.socialMediaChange.emit(updated);
    }
}
